...
/**
 * Spring Security config: stateless JWT-in-HttpOnly-cookie auth (see CLAUDE.md — Auth section).
 *
 * <p>CSRF stays disabled — {@code SameSite=Strict} on the {@code jwt} cookie already stops it
 * being sent on cross-site requests, which neutralizes the classic CSRF vector (see ARCHI.md —
...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        JwtAuthenticationFilter jwtAuthenticationFilter = new JwtAuthenticationFilter(
                authenticationManager, jwtTokenService, objectMapper, validator, userMapper, secureCookie);
        ...
        http
                ...
                .exceptionHandling(ex -> ex
                        ...
                        .accessDeniedHandler((request, response, accessDeniedException) ->
                                writeJsonError(response, HttpServletResponse.SC_FORBIDDEN, "Droits insuffisants")))
    ...
    // JwtAuthorizationFilter is also a @Component so Spring Boot would otherwise auto-register
    // it a second time as a plain servlet filter (outside the security chain) via
    // ServletContextInitializerBeans — harmless today since it's idempotent, but disabled
