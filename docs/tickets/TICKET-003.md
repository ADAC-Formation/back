...
- `src/main/java/com/adac/portail/entity/ActivationToken.java` — avec `TokenType type` et `OffsetDateTime usedAt`
  (nullable). Colonne `code` renommée `code_hash` + ajout `attempts` (review TICKET-003 : un code à 6 chiffres
  en clair en base est directement exploitable pour un takeover ; le hash + compteur de tentatives sera
  branché dans TICKET-015)
...
- `src/main/java/com/adac/portail/entity/enums/NotificationType.java` — `NEW_MESSAGE`, `DOCUMENT_UPLOADED`,
  `FORMATION_UPDATED` (corrigé pendant l'implémentation — `DB_MODEL.mmd`, `DB_MODEL.md` et `tech.md`
  s'accordaient déjà sur ces valeurs, seul ce ticket listait `MESSAGE`/`DOCUMENT`/`FORMATION`)
...
- [ ] `ActivationToken.usedAt` est `OffsetDateTime` nullable (pas un boolean) — `OffsetDateTime`, pas
  `LocalDateTime`, car toutes les colonnes sont `TIMESTAMPTZ` (voir review TICKET-003 : `LocalDateTime` sur
  une colonne `timestamptz` décale silencieusement les valeurs selon le fuseau JVM)
